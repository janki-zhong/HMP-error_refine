...
def pre_normalization(data, zaxis=[0, 1], xaxis=[8, 4]):
    ...
    for i_s, skeleton in enumerate(tqdm(s)):
        ...
        for i_p, person in enumerate(skeleton):
            ...
            # person = person.transpose.(0, 2, 3, 1)
            # for i_j, joint in enumerate(person):
            #     s[i_s, i_p, i_j, :] = np.dot(matrix_z, joint)
...
if __name__ == '__main__':
    data = np.load('/home/user0/Public/yyx/pywork/dataset/cmu/train/data_joint.npy')
    pre_normalization(data)
    np.save('/home/user0/Public/yyx/pywork/dataset/cmu/train/data_joint_pre.npy', data)
