...
from utils.skeleton import Feeder
from utils.preprocess import pre_normalization
...
def readCSVasFloat(filename):
    ...
    """
    returnArray = []
    lines = open(filename).readlines() # 把一个文档的每一行行末加一个\n，将每行作为列表的一个元素，存储在一个list中。
    for line in lines:
        line = line.strip().split(',') #消除右边空格
        if len(line) > 0:
...
def load_data_cmu_3d_n(opt, path_to_dataset, actions, input_n, output_n, is_test=False):
    ...
    seq_len = input_n + output_n
    nactions = len(actions) #每次只有一个动作进来 长度1
    sampled_seq = []
    complete_seq = []
    for action_idx in np.arange(nactions):
        action = actions[action_idx]
        ...
        for examp_index in np.arange(count):
            ...
            #is_test=        or   -1<0=True =True(一定走这里)
    ...
    # sampled_seq_pre = sampled_seq[:, :, dimensions_to_use]

    # sampled_seq_pre = sampled_seq.reshape(sampled_seq.shape[0], seq_len, 3, -1)
    # print(sampled_seq_pre.shape)
    # sampled_seq_pre = np.expand_dims(sampled_seq_pre, axis=0)
    # sampled_seq_pre = sampled_seq_pre.transpose(1, 3, 2, 4, 0)
    #
    # print(sampled_seq_pre.shape)
    #
    # save_filename_data = '{}/data_joint.npy'.format(path_to_dataset)
    # np.save(save_filename_data, sampled_seq_pre)
    #
    # data = np.load(save_filename_data)
    # data = pre_normalization(data)
    #
    # sampled_seq = data.reshape(sampled_seq.shape[0], sampled_seq.shape[1], -1)


    # sampled_seq[:, :, dimensions_to_use] = data[:, :, :]
    # -------------------------------------------------------------
    ...
    # # ----------------------------
    # joint_to_ignore_1 = np.array([0, 3, 4, 5, 6, 7, 9, 10, 11, 12, 13, 16, 20, 29, 22,23,24, 27, 25,26,28,  31,32,33,  36,  34,35,37])
    # dimensions_to_ignore_1 = np.concatenate((joint_to_ignore * 3, joint_to_ignore * 3 + 1, joint_to_ignore * 3 + 2))
    # dimensions_to_use_1 = np.setdiff1d(np.arange(complete_seq.shape[1]), dimensions_to_ignore)
    # joint_to_ignore_2 = np.array([ 0, 4, 5, 6, 7, 10, 11, 12, 13, 16, 20, 29,23,24, 27, 25,26,28,  32,33, 36,  34,35,37])
    # dimensions_to_ignore_2 = np.concatenate((joint_to_ignore * 3, joint_to_ignore * 3 + 1, joint_to_ignore * 3 + 2))
    # dimensions_to_use_2 = np.setdiff1d(np.arange(complete_seq.shape[1]), dimensions_to_ignore)
    #
    #
    #
    # joint_to_ignore_3 = np.array([ 0,7,13,16, 20, 29, 24, 27, 33, 36])
    # dimensions_to_ignore_3 = np.concatenate((joint_to_ignore * 3, joint_to_ignore * 3 + 1, joint_to_ignore * 3 + 2))
    # dimensions_to_use_3 = np.setdiff1d(np.arange(complete_seq.shape[1]), dimensions_to_ignore)
    #
    #
    #
    # # ----------------------------
...
def load_data_cmu_3d_all(opt, path_to_dataset, actions, input_n, output_n, is_test=False):
    ...
    dimensions_to_ignore = np.concatenate((joint_to_ignore * 3, joint_to_ignore * 3 + 1, joint_to_ignore * 3 + 2))
    dimensions_to_use = np.setdiff1d(np.arange(complete_seq.shape[1]), dimensions_to_ignore)
    #np.setdiff1d找出两个数组中元素的差异（（38*3）114-13*3=75）
    ...
    #
    # # ----------------------------------------------------
    # data_pre = data.reshape(sampled_seq.shape[0], seq_len,-1)
    # data_pre = data_pre[:, :, dimensions_to_use]
    #
    # data_pre = data_pre.reshape(sampled_seq.shape[0], seq_len, 3, -1)
    # print(data_pre.shape)
    # data_pre = np.expand_dims(data_pre, axis=0)
    # data_pre = data_pre.transpose(1, 3, 2, 4, 0)
    # #----------------------------------------------------
    #
    #
    # save_filename_datapre = '{}/data_joint_pre.npy'.format(path_to_dataset)
    # np.save(save_filename_datapre, data_pre)
    #
    #
    #
    #
    #
    # S = Feeder(save_filename_datapre, [0.1, 1], 64,"seq-based_and_graph-based" )
    # sampled_seq_aug = []
    # # b = []
    ...
    #     a, _= S[i]
    #     a = a.tolist()
    #     sampled_seq_aug.append(a)
    #     # a=np.concatenate((a,b),axis=0)
    #
    # # a, _, _, _ = S[1]
    # # a, _, _, _ = S
    # sampled_seq_aug = np.array(sampled_seq_aug)
    # print(sampled_seq_aug.shape)
    #
    #
    # sampled_seq[:, :, dimensions_to_use] = sampled_seq_aug[:, :]
    ...
    # #------------------------------------------------------------------------
    # joint_to_ignore_1 = np.array(
    #     [0, 3, 4, 5, 6, 7, 9, 10, 11, 12, 13, 16, 20, 29, 22, 23, 24, 27, 25, 26, 28, 31, 32, 33, 36, 34, 35, 37])
    # dimensions_to_ignore_1 = np.concatenate((joint_to_ignore * 3, joint_to_ignore * 3 + 1, joint_to_ignore * 3 + 2))
    # dimensions_to_use_1 = np.setdiff1d(np.arange(complete_seq.shape[1]), dimensions_to_ignore)
    # joint_to_ignore_2 = np.array(
    #     [0, 4, 5, 6, 7, 10, 11, 12, 13, 16, 20, 29, 23, 24, 27, 25, 26, 28, 32, 33, 36, 34, 35, 37])
    # dimensions_to_ignore_2 = np.concatenate((joint_to_ignore * 3, joint_to_ignore * 3 + 1, joint_to_ignore * 3 + 2))
    # dimensions_to_use_2 = np.setdiff1d(np.arange(complete_seq.shape[1]), dimensions_to_ignore)
    #
    # joint_to_ignore_3 = np.array([0, 7, 13, 16, 20, 29, 24, 27, 33, 36])
