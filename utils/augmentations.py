...
def joint_courruption(input_data):                                                                     
    ...
    D = np.setdiff1d(np.arange(V), joint_to_ignore)
    ...
    if flip_prob < 0.5:
        ...
        joint_indicies = np.random.choice(D, D.size-10,replace=False) #从A中随机抽选B个数字组成一个新数组
        out[:,:,joint_indicies,:] = 0 
        return out
    
    else:
         #joint_indicies = np.random.choice(25, random.randint(5, 10), replace=False)
         joint_indicies = np.random.choice(D, D.size-10,replace=False)
...
def temporal_cropresize(input_data,num_of_frames,l_ratio,output_size):
    ...
    temporal_context = temporal_context.squeeze(dim=2).squeeze(dim=0)
